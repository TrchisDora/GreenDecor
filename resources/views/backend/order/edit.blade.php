                        ...
                        <form action="{{ route('order.update', $order->id) }}" method="POST">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select name="status" class="form-control" {{ in_array($order->status, ['delivered', 'store_payment', 'cancelled', 'failed_delivery', 'out_of_stock']) ? 'disabled' : '' }}>
                                    @if($order->status == 'new')
                                        <option value="process" {{ old('status') == 'process' ? 'selected' : '' }}>Đã xử lý
                                            đơn hàng</option>
                                        <option value="out_of_stock" {{ old('status') == 'out_of_stock' ? 'selected' : '' }}>Hết
                                            hàng</option>
                                    @elseif($order->status == 'cancel_requested')
                                        <option value="cancel_requested" {{ old('status') == 'cancel_requested' ? 'selected' : '' }}>Xác nhận yêu cầu hủy đơn</option>
                                    @elseif($order->status == 'process')
                                        <option value="shipping" {{ old('status') == 'shipping' ? 'selected' : '' }}>Đang giao
                                            hàng</option>
                                    @elseif($order->status == 'shipping')
                                        <option value="delivered" {{ old('status') == 'delivered' ? 'selected' : '' }}>Giao hàng
                                            thành công</option>
                                        <option value="failed_delivery" {{ old('status') == 'failed_delivery' ? 'selected' : '' }}>Giao hàng thất bại</option>
                                    @endif
                                </select>
                                @error('status')
